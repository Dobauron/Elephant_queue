...
def get_excel_data(row, col):  # Function used to connect with excel file
...
def list_boss_and_basic_creator():  # function contain loop which taking data from excel sheet and
    ...
    for row in range(1, get_excel_data(2, 1)+1):
        eleph = Elephant(row, get_excel_data(row+1, 2)) # object creation, according excel sheet, argument(mass, number)

        list_elephant.append(eleph)  # list of elephant object
        non_boss_list.append(get_excel_data(row + 1, 3))  # first elephant queue
        Elephant.boss_queue.append(get_excel_data(row + 1, 4))  # elephant queue according boss demands
